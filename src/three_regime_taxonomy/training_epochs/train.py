...
parser.add_argument('--schedule', type=int, nargs='+', default=[80, 120], help='Decrease learning rate at these epochs.')
...
parser.add_argument('--save_dir', default='/work/yefan0726/checkpoints/cifar/resnet20', type=str)
parser.add_argument('--data-path', default='/work/yefan0726/data/cifar', type=str)
parser.add_argument('--epoch-stop-intervel', default=10, type=int)
...
def main():
    if os.path.exists(os.path.join(args.save_dir, f"checkpoint_epoch_{args.epochs}.pth.tar")):
    ...
    if not os.path.isdir(args.save_dir):
        ...
        logging.basicConfig(filename=os.path.join(args.save_dir, 'args.log'), level=logging.DEBUG)
        logging.info(args.__dict__)

    # Data
    trainloader, testloader, num_classes = get_dataloader(args)
    ...
    if args.resume:
        # Load checkpoint.
        print(f"==> Resuming from checkpoint..{args.resume}, start_epoch: {checkpoint['epoch']}")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.optimizer == 'SAM':
            train_loss, train_acc = train_sam(trainloader, model, criterion, optimizer, epoch, use_cuda)
        else:
...
def train_sam(trainloader, model, criterion, optimizer, epoch, use_cuda):
    # switch to train mode
    model.train()
...
def save_checkpoint(state, checkpoint, filename='checkpoint.pth.tar'):
    ...
    if state['epoch'] % args.epoch_stop_intervel == 0:
        print("Saving", filepath)
        torch.save(state, filepath)
        shutil.copyfile(filepath, os.path.join(checkpoint, f"checkpoint_epoch_{state['epoch']}.pth.tar"))
