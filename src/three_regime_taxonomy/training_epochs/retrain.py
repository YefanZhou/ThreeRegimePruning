...
from three_regime_taxonomy.utils import get_conv_zero_param, get_dataloader, get_model, Bar, Logger, AverageMeter, accuracy, mkdir_p
...
parser.add_argument("--epochs_to_save", nargs='+', type=int, default=[2])
...
def main():
    ...
    logger.close()
    finetune_stats_dict = {'before_finetune_test_acc':before_test_acc, 'before_finetune_test_loss':before_test_loss, 
                    'best_acc': best_acc, 
...
def save_checkpoint(state, is_best, checkpoint, filename='finetuned.pth.tar', end_of_training=False):
    ...
    if args.epochs_to_save:
        filepath = os.path.join(checkpoint, filename)
        torch.save(state, filepath)
        if state['epoch'] in args.epochs_to_save == 0:
