...
class Bezier(Module):
    def __init__(self, num_bends):
        ...
        self.register_buffer(
            'binom',
            torch.Tensor(binom(num_bends - 1, np.arange(num_bends), dtype=np.float32))  # 1 2 1
        )
        self.register_buffer('range', torch.arange(0, float(num_bends)))   # 0 1 2
        self.register_buffer('rev_range', torch.arange(float(num_bends - 1), -1, -1))  # 2 1 0

    def forward(self, t):
        # phi_theta = (1 - t)^2 w1 + 2t(1-t)*theta + t^2 w2
        return self.binom * \
...
class Conv2d(CurveModule):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, fix_points, stride=1,
        ...
        Args:
            inverse_weight_bias (bool, optional): inverse order of register weight and bias
...
class CurveNet(Module):
    ...
    def import_base_parameters(self, base_model, index):
        # count_idx = 0
        # num_conv = 0
        # for name, param in list(self.net.named_parameters())[0::3]:
        #     print(name, count_idx, param.shape)
        #     if 'conv' in name:
        #         num_conv += 1
        #     count_idx += 1
        # count_idx = 0
        ...
        # num_conv = 0
        # for name, param in list(base_model.named_parameters()):
        #     print(name, count_idx, param.shape)
        #     if 'conv' in name:
        #         num_conv += 1
        #     count_idx += 1
        ...
        # import sys
        # sys.exit(0)
        #print(index, self.num_bends, len(list(self.net.parameters())))
        ...
        for parameter, base_parameter in zip(parameters, base_parameters):
            #print(parameter.shape, base_parameter.shape)
...
def get_curve_prune_modules(model):
    modules = []
    for m in model.modules():
        if _is_curve_prunable_module(m):
            modules.append(m)
    return modules


def _is_curve_prunable_module(m):
    return (isinstance(m, Linear) or isinstance(m, Conv2d))