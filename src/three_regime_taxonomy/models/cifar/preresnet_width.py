...
class PreResNetWidth(nn.Module):

    def __init__(self, depth, k=1, num_classes=10):
        super(PreResNetWidth, self).__init__()
        ...
        self.inplanes = int(16 * k)
        self.conv1 = nn.Conv2d(3, int(16 * k), kernel_size=3, padding=1, bias=False)
        self.layer1 = self._make_layer(block, int(16 * k), n)
        self.layer2 = self._make_layer(block, int(32 * k), n, stride=2)
        self.layer3 = self._make_layer(block, int(64 * k), n, stride=2)
        self.bn = nn.BatchNorm2d(int(64 * k * block.expansion))
        self.relu = nn.ReLU(inplace=True)
        self.avgpool = nn.AvgPool2d(8)
        self.fc = nn.Linear(int(64 * k * block.expansion), num_classes)
...
class PreResNetCurveWidth(nn.Module):

    def __init__(self, num_classes, fix_points, depth=110, k=1):
        super(PreResNetCurveWidth, self).__init__()
        n = (depth - 2) // 6

        block = BasicBlockCurve

        self.inplanes = int(16 * k)
        self.conv1 = curves.Conv2d(3, int(16 * k), kernel_size=3, padding=1,
                                   bias=False, fix_points=fix_points)
        self.layer1 = self._make_layer(block, int(16 * k), n, fix_points=fix_points)
        self.layer2 = self._make_layer(block, int(32 * k), n, stride=2, fix_points=fix_points)
        self.layer3 = self._make_layer(block, int(64 * k), n, stride=2, fix_points=fix_points)
        self.bn = curves.BatchNorm2d(int(64 * k * block.expansion), fix_points=fix_points)
        self.relu = nn.ReLU(inplace=True)
        self.avgpool = nn.AvgPool2d(8)
        self.fc = curves.Linear(int(64 * k * block.expansion), num_classes, fix_points=fix_points)
...
def preresnet_width(**kwargs):
    """
    Constructs a ResNet model.
    """
    
    return PreResNetWidth(**kwargs)




class preresnet_width_curve:
    curve = PreResNetCurveWidth
