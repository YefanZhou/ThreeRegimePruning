...
for TASK_ID in $(seq 1 "$end")
    do
        ...
        epochs=$(echo $cfg | cut -f 16 -d ' ')
        retrain_batch=$(echo $cfg | cut -f 17 -d ' ')
        num_iterations=$(echo $cfg | cut -f 18 -d ' ')
        rho=$(echo $cfg | cut -f 19 -d ' ')
        retrain_optimizer=$(echo $cfg | cut -f 20 -d ' ')
        ...
        load_dir=${load_dir}/${prune_distri}/p_ratio_${prune_ratio}/pruned.pth.tar
        save_dir=${save_dir}/${prune_distri}/p_ratio_${prune_ratio}/retrain_epoch${epochs}/retrain_seed${retrain_seed}
