...
for TASK_ID in $(seq 1 "$end")
    do
        ...
        num_iterations=$(echo $cfg | cut -f 15 -d ' ')
        ...
        if [ "$optimizer" = "SAM" ]; then
            ...
            load_dir=${ckpt_path}/${task}/${dataset}/${last_stage}/${optimizer}_rho${rho}/lr_${initial_lr}/${arch}${depth}_width${width_scale}_seed${seed}_batchsize${batch_size}/checkpoint_iter_${num_iterations}.pth.tar
            ...
            load_dir=${ckpt_path}/${task}/${dataset}/${last_stage}/${optimizer}/lr_${initial_lr}/${arch}${depth}_width${width_scale}_seed${seed}_batchsize${batch_size}/checkpoint_iter_${num_iterations}.pth.tar
        ...
        save_dir=${save_dir}/${prune_distri}/p_ratio_${prune_ratio}
